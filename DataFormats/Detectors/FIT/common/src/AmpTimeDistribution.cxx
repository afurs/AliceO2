...
std::size_t AmpTimeDistribution::serialize(std::vector<AmpTimeDistribution::Content_t>& vecDst, size_t startPos)
{
  vecDst.emplace_back(static_cast<AmpTimeDistribution::Content_t>(mHist->GetEntries()));
  ++startPos;
  const size_t cnt = sizeof(AmpTimeDistribution::Content_t) * mHist->GetNcells();
  std::memcpy(&vecDst[startPos], mHist->GetArray(), cnt);
  startPos += mHist->GetNcells();
  return startPos;
}

std::size_t AmpTimeDistribution::deserialize(std::vector<AmpTimeDistribution::Content_t>& vecSrc, size_t startPos)
{
  mHist->SetEntries(static_cast<double>(vecSrc[startPos++]));
  const size_t cnt = sizeof(AmpTimeDistribution::Content_t) * mHist->GetNcells();
  std::memcpy(mHist->GetArray(), &vecSrc[startPos], cnt);
  startPos += mHist->GetNcells();
  return startPos;
}

std::size_t AmpTimeDistribution::addContent(const std::span<const AmpTimeDistribution::Content_t>& vecSrc, size_t startPos)
{
  const auto nEntries = mHist->GetEntries() + vecSrc[startPos++];
  std::size_t nCells = mHist->GetNcells();
  std::size_t iCell = 0;
  auto arrDst = mHist->GetArray();
  const auto vecSize = VcContent_t::size();
  // Process elements in vectorized chunks
  for (; iCell + vecSize <= nCells; iCell += vecSize) {
    VcContent_t vecChunkSrc(&vecSrc[iCell + startPos], Vc::Aligned);
    VcContent_t vecChunkDst(&arrDst[iCell], Vc::Aligned);
    vecChunkDst += vecChunkSrc;
    vecChunkDst.store(&arrDst[iCell], Vc::Aligned);
  }
  for (; iCell < vecSize; ++iCell) {
    const auto val = vecSrc[iCell + startPos];
    arrDst[iCell] += val;
  }
  mHist->SetEntries(nEntries);
  return startPos + nCells;
}